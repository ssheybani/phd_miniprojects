...
#     traj_specs = [[1, 10,2], [1, 25,0], [0.5,35,1], [0.2,100,0.5], [0.2, 150,1.3]]
#     traj_specs = [[1, 0.3,2], [1, 0.6,0], [0.5,0.9,1], [0.2,1.5,0.5], [0.2, 2.1,1.3]]
#     traj_specs = [[1, 0.027,1.2], [4, 0.05,0.5], [0.5,0.011,1], [0.2,0.012,0.9], [2.5, 0.021,1.3]]
#     traj_specs = [[1, 0.03,2], [1, 0.06,0], [0.5,0.009,1], [0.2,0.015,0.5], [0.2, 0.021,1.3]]
    if traj_specs==None:
...
class ushaped_f_obsolete():
    # For the penalty function of the egg
    def __init__(self, lb, ub, fmax, flat_perc=0.7):
        
        flat_p_h = flat_perc/2
        r = ub-lb
        self.a = 7*fmax /((1-flat_p_h**2)*r*r)
        self.b = -self.a*(flat_p_h*r)**2
        self.m = (ub+lb)/2
        
    def apply(self, x):
        val = self.a*(x-self.m)**2 +self.b
        if val>0:
            return val
        return 0.
...
def bound(x, bnd, symm=True):
    ...
    # if symm=False, bnd should be a sequence like [upper bound, lower bound]. 
...
def myinterp(ind):
    def _interp(x):
        ...
        alpha = 0.1
        slope = 150.*(alpha+1)
        return slope*x/(alpha+abs(x))
    return [_interp(x) for x in _get_iterable(ind)]

# def myinterp2(ind, debug=False):
#     # This interpolator is created to map n+1 parameters 
#     # within [0,1] to n parameters within [-w_gain, w_gain].
#     # 'w_gain' is calculated using the first element as well as an internal variable, 'slope'.
# #     Note that the output array is smaller than the input by one unit.
#     small_gain = interp1d([0,1], [-1,1])
#     out = narr([small_gain(x) for x in get_iterable(ind)])
#     if debug==True:
#         print('out=', out)

#     w_gain =map_w_gain*out[0]
# #     alpha = 0.1; slope = 150.*(alpha+1);
# #     w_gain = slope*out[0]/(alpha+abs(out[0]))
