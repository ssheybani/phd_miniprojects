...
# Agent 2: pushes towards the negative direction
# This file is identical to agent1.py file, except for two things:
# 1- agent_dual_mode.sensor2feature() uses the negative of ref and err.
# 2- The output force is published on topic 'force_down'
# 3- The node name is obviously different.
...
if __name__ == '__main__':
    ...
    test_paramset = sols[2][2*n_w:4*n_w]+sols[2][-2:]#switchCondpos+switchCondneg+[1, 1]



    # Create publishers and subscribers
    # mrkr_pub = rospy.Publisher('visualization_marker_array', MarkerArray,  queue_size=10)
    force_pub = rospy.Publisher('force_down', ForceType, queue_size=1)
    ...
    # Setup rospy
    rospy.init_node('agent2_node', anonymous=False)
    fps = 1./tstep
    rate = rospy.Rate(fps) 
