...
//
// The code was provided by Randall D. Beer.
// http://pages.iu.edu/~rdbeer/
// *******************************************************************************
...
void TSearch::SetVectorSize(int NewSize)
{
	...
	if (NewSize <= 0) {cout << "Invalid vector size: "<< NewSize; exit(0);}
	vectorSize = NewSize;
...
{
	if (NewSize <= 0) {cout << "Invalid population size: "<< NewSize; exit(0);}
...
{
	if (NewMax < 0) {
		cout << "Invalid MaxGenerations: " << NewMax;
		exit(0);
	}
	MaxGens = NewMax;
}
...
void TSearch::SetElitistFraction(double NewFraction)
{
	if (NewFraction < 0.0 || NewFraction > 1.0) {
		cout << "Invalid ElitismFraction: " << NewFraction;
		exit(0);
	}
...
void TSearch::SetMaxExpectedOffspring(double NewVal)
{
	if (NewVal < 1.0 || NewVal > 2.0) {
		cout << "Invalid MaxExpectedOffspring: " << NewVal;
		exit(0);
	}
...
{
	if (NewVariance <= 0.0) {
		cout << "Invalid MutationVariance: " << NewVariance;
		exit(0);
	}
...
void TSearch::SetCrossoverProbability(double NewProb)
{
	if (NewProb < 0.0 || NewProb > 1.0) {
		cout << "Invalid CrossoverProbability: " << NewProb;
		exit(0);
	}
	CrossProb = NewProb;
}
...
void TSearch::SetCrossoverTemplate(TVector<int> &NewTemplate)
{
	...
	if (NewTemplate.Size() != vectorSize) {
		cout << "Invalid vector size for CrossoverTemplate: " << NewTemplate.Size();
		exit(0);
	}
	int x = 1;
	for (int i = 1; i <= NewTemplate.Size(); i++)
		if (NewTemplate[i] != x)
			if (NewTemplate[i] == x+1) x++;
				...
				cout << "Invalid format for CrossoverTemplate: " << NewTemplate;
				exit(0);
			}
...
void TSearch::SetCrossoverPoints(TVector<int> &NewPoints)
{
	...
	if (NewPoints.Size() > vectorSize) {
		cout << "Invalid vector size for Crossover Points: " << NewPoints.Size();
		exit(0);
	}
	if (NewPoints.Size() < 1 || NewPoints[1] != 1) {
		cout << "Invalid format for Crossover Points: " << NewPoints;
		exit(0);
	}
	int x = 0;
...
void TSearch::SetSearchConstraint(TVector<int> &constraint)
{
	if (constraint.Size() != vectorSize) {
		cout << "Invalid vector size for SearchConstraint: " << constraint;
		exit(0);
	}
...
{
	if (NewInterval < 0) {
		cout << "Invalid CheckpointInterval: " << NewInterval;
		exit(0);
	}
...
void TSearch::DoSearch(int ResumeFlag)
{
	...
	{
		cout << "Error: NULL evaluation function\n";
		exit(0);
	}
...
void TSearch::EvaluatePopulation(int start)
{
  ...
  if (THREAD_COUNT > 1) {
    ...
    for (int i = 1; i <= THREAD_COUNT - 1; i++) {
      ...
      if (rc) {cout << "Thread creation failed: " << rc << endl; exit(-1);}
    }
...
void TSearch::UpdatePopulationFitness(void)
{
	...
	switch (SelectMode) {
		...
		case RANK_BASED:
			...
			break;
		default: cout << "Invalid selection mode" << endl; exit(0);
	}
}
...
void TSearch::ReproducePopulation(void)
{
	switch (RepMode) {
		...
		default: cout << "Invalid reproduction mode" << endl; exit(0);
	}
}
	...
	while (i <= psize) {
		...
		if (ProbabilisticChoice(CrossProb) && (i < psize)) {
			...
			switch (CrossMode) {
				...
				default: cout << "Invalid crossover mode" << endl; exit(0);
			}
